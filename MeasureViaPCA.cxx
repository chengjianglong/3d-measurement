...
int main(int argc, char *argv[])
{
  ...
  //----------------------------------------------------------------------/
  // Parse the point information and run PCA to extract X, Y axis.
  //----------------------------------------------------------------------/
  //
  ...
  vtkSmartPointer<vtkTransform> PCA_transform = vtkSmartPointer<vtkTransform>::New();
  PCA_transform->RotateZ(30);
  
  vtkSmartPointer<vtkTransformPolyDataFilter> PCA_transformFilter = vtkSmartPointer<vtkTransformPolyDataFilter>::New();
  PCA_transformFilter->SetTransform(PCA_transform);
#if VTK_MAJOR_VERSION <= 5
  PCA_transformFilter->SetInputConnection(polydata->GetProducerPort());
#else
  PCA_transformFilter->SetInputData(mypolydata);
#endif
  PCA_transformFilter->Update();
  ...
  {
    double p[3];
    PCA_transformFilter->GetOutput()->GetPoint(i,p);
  ...
  xNormal[0] = evec1->GetValue(0);
  xNormal[1] = evec1->GetValue(1);
  xNormal[2] = evec1->GetValue(2);
  ...
  std::cout << std::endl << "------------- Point ---------" << std::endl;
  //vtkSmartPointer<vtkPolyData> output =  reader->GetOutput();
  vtkSmartPointer<vtkPolyData> new_output =  transformFilter->GetOutput();
  std::cout << "point # : " << new_output->GetNumberOfPoints() << std::endl;
  for(int i=0; i<new_output->GetNumberOfPoints(); i++)
  {
	  double p[3];
	  new_output->GetPoint(i,p);
